...
func LoggerMiddleware() gin.HandlerFunc {
	logsPath := "./internal/logs/http.log"
	...
	return func(ctx *gin.Context) {
		...
		if strings.HasPrefix(contentType, "multipart/form-data") {
			maxDefaultContent := 32 << 20
			if err := ctx.Request.ParseMultipartForm(int64(maxDefaultContent)); err != nil && ctx.Request.MultipartForm != nil {
			...
			if err != nil {
				logger.Error().Err(err).Msg("Unable to read the request body")
			}
